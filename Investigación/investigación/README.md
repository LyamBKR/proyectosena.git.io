# 📊 Carpeta de Investigación

Esta carpeta contiene los archivos Excel relacionados con la investigación del proyecto sobre la industria de videojuegos en Colombia.

## 📁 Contenido

Los archivos Excel con datos, análisis y reportes de investigación se almacenan aquí.

## 📥 Descarga

Utiliza el botón "Descargar Excel" en la página de Investigación para acceder a los archivos disponibles.

## 🔬 Módulo de Investigación

### Título del Proyecto
**¿Qué sucede con los videojuegos en Colombia?**

### Descripción
Investigación formativa sobre el estado actual, características, desafíos y oportunidades del sector de videojuegos en Colombia para el año 2025.

### Objetivos Específicos
1. Caracterizar los principales actores de la industria de videojuegos en Colombia
2. Identificar oportunidades, desafíos y riesgos para el desarrollo sostenible del sector

### 📊 Recolección de la Información

#### Técnicas Utilizadas
- **Encuestas**: Cuestionario digital mediante Microsoft Forms
- **Entrevistas**: Guías semiestructuradas para líderes del sector

#### Instrumentos
- **Cuestionario Digital (Microsoft Forms)**: Para jugadores y participantes de eventos
- **Guía de entrevista**: Para expertos y líderes del sector

#### Características
- Preguntas cerradas y abiertas sobre consumo, percepción y retos del sector
- Aplicación virtual para llegar a participantes en Bogotá
- Permite análisis cuantitativo y cualitativo

### 📋 Encuesta: "¿Cómo está la industria en este país en 2025 - 2026?"

**Elaborado por**: Lyam Acosta Forero

**Objetivo**: Identificar la realidad de jugadores y empresas en Colombia, tendencias, percepciones y retos para el desarrollo sostenible del sector.

**Instrucciones**: Solicitar a jugadores de eventos (ej. Colombia 4.0) y comunidades que respondan sobre hábitos de consumo, percepción de la industria, riesgos y perspectivas.

#### Desarrollo del Instrumento (Preguntas Principales)
1. ¿Cuántas horas juegas videojuegos a la semana?
2. ¿Qué plataforma usas principalmente? (A: PC / B: Consolas / C: Celulares / D: Otros)
3. ¿Conoces videojuegos desarrollados en Colombia? (A: Si / B: No)
4. ¿Has jugado alguno?
5. ¿Qué consideras el mayor reto para los desarrolladores nacionales?
6. ¿Qué impacto tienen los videojuegos en tus rutinas familiares o personales?
7. ¿Participas en comunidades de juegos aquí o en otro lado? (Aquí: Bogotá / Otro lado: Internet si no vive fuera del país)
8. ¿Crees que el sector crecerá en los próximos años? ¿Por qué?

### 🎤 Entrevistas para Expertos

#### Preguntas Sugeridas
1. ¿Conoces iniciativas que apoyen el desarrollo de videojuegos en el país?
2. ¿Qué obstáculos enfrentan las empresas locales para internacionalizar su producto?
3. ¿Cómo ves la industria desde tu perspectiva?
4. ¿Cómo ves la industria en los próximos años?

### 🔗 Enlace de la Encuesta

**Microsoft Forms**: https://forms.office.com/r/XrDwE6QVKe

---

## 📈 ANÁLISIS DE DATOS

### Descripción

En esta etapa se deben analizar e interpretar los datos recogidos en la fase 2 de ejecución.

### Resultados de la Encuesta - Colombia 4.0 Bogotá

Se realizaron **100 encuestas** en el evento Colombia 4.0 en Bogotá, donde se descubrió que el **73% juega en dispositivos móviles**, el **18% en consola** y el **9% en PC**. Adicionalmente descubrimos que el tiempo promedio semanal es del **8%** y el **40% ha comprado y descargado juegos desarrollados aquí**, y ese mismo porcentaje identifica el reto para la industria en Colombia por la falta de inversión y apoyo del gobierno. Adicionalmente descubrimos una plataforma que apoya a los desarrolladores de videojuegos llamada **"Tan Grande y Jugando"**, y solo el **10% participa en alguna comunidad**. La compra promedio entre los jugadores fue de **$70.000 a $100.000**.

### 📊 Estadísticas Clave

| Métrica | Porcentaje/Valor | Descripción |
|---------|------------------|-------------|
| 📱 Dispositivos Móviles | **73%** | Porcentaje de jugadores que juegan en móviles |
| 🎮 Consolas | **18%** | Porcentaje de jugadores que juegan en consolas |
| 💻 PC | **9%** | Porcentaje de jugadores que juegan en PC |
| 🛒 Compra de Juegos Nacionales | **40%** | Porcentaje que ha comprado/descargado juegos desarrollados en Colombia |
| ⚠️ Reto Principal | **40%** | Identifican falta de inversión y apoyo del gobierno |
| 👥 Participación en Comunidades | **10%** | Porcentaje que participa en alguna comunidad |
| 💰 Compra Promedio | **$70.000 - $100.000** | Rango de compra promedio entre jugadores |
| ⏰ Tiempo Promedio Semanal | **8%** | Tiempo promedio de juego semanal |

### Hallazgos Importantes

1. **Dominancia del móvil**: El 73% de los encuestados juega principalmente en dispositivos móviles, lo que indica una tendencia clara hacia el gaming móvil en Colombia.

2. **Apoyo a desarrolladores locales**: El 40% ha comprado o descargado juegos desarrollados en Colombia, mostrando un interés en el contenido local.

3. **Retos identificados**: El 40% de los encuestados identifica la falta de inversión y apoyo gubernamental como el principal reto para la industria.

4. **Plataforma de apoyo**: Se descubrió la plataforma **"Tan Grande y Jugando"** que apoya a desarrolladores de videojuegos.

5. **Baja participación en comunidades**: Solo el 10% participa en comunidades de juegos, indicando una oportunidad de crecimiento.

6. **Gasto promedio**: Los jugadores invierten entre $70.000 y $100.000 pesos colombianos en promedio en videojuegos.

---

## 📝 Notas

- Los archivos Excel deben tener formato `.xlsx`
- Mantén los archivos organizados por fecha o tema
- Actualiza este README cuando agregues nuevos archivos
- El diseño visual de la página incluye estilos CSS personalizados para tablas, listas de preguntas y contenedores

## 🎨 Estilos CSS

La página de investigación utiliza estilos CSS personalizados incluidos en `css/style.css`:
- `.investigation-section-header`: Encabezados con fondo verde oscuro
- `.investigation-table`: Tablas con bordes visibles y efectos hover
- `.investigation-questions-list`: Listas numeradas estilizadas con tarjetas
- `.survey-link-container`: Contenedor destacado para el enlace de la encuesta

## 👤 Autor

**Desarrollador**: Lyam Acosta Forero  
**Proyecto**: SENA - Análisis y desarrollo de software  
**Año**: 2025-2026